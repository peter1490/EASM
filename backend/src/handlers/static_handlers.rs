...
pub async fn serve_evidence_file(
    ...
    let evidence_dir = PathBuf::from(&settings.evidence_storage_path);
    ...
    if metadata.len() > settings.max_evidence_bytes {
        return Err(ApiError::validation("File too large"));
    }

    // Read file content
    let file_content = fs::read(&canonical_file_path)
        .await
    ...
    if !is_content_type_allowed(&content_type, &settings.evidence_allowed_types) {
...
pub async fn static_files_health_check(
    State(app_state): State<AppState>,
) -> Result<&'static str, ApiError> {
    // Check if evidence directory exists and is accessible
    let evidence_dir = PathBuf::from(&app_state.config.load().evidence_storage_path);
    ...
    match fs::metadata(&evidence_dir).await {
        ...
        Err(e) => Err(ApiError::internal(format!(
            "Evidence directory not accessible: {}",
            e
        ))),
    }
}
...
mod tests {
    ...
    async fn test_serve_evidence_file_not_found() {
        ...
        let db_pool = crate::database::create_database_pool(&settings.database_url)
            .await
            .expect("DB pool");
        let app_state = AppState::new_with_pool(settings, db_pool).await.unwrap();

        let result =
            serve_evidence_file(State(app_state), Path("nonexistent.txt".to_string())).await;
    ...
    async fn test_serve_evidence_file_success() {
        ...
        let result = serve_evidence_file(State(app_state), Path("test.txt".to_string())).await;
    ...
    async fn test_serve_evidence_file_path_traversal() {
        ...
        let result =
            serve_evidence_file(State(app_state), Path("../../../etc/passwd".to_string())).await;
