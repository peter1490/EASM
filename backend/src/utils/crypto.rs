...
pub async fn get_tls_certificate_info(hostname: &str, port: u16) -> Result<TlsCertificateInfo, ApiError> {
    ...
    config.dangerous().set_certificate_verifier(cert_capture.clone());
    ...
    let tcp_stream = TcpStream::connect(socket_addr).await
        .map_err(|e| ApiError::ExternalService(format!("Failed to connect to {}:{}: {}", hostname, port, e)))?;
    ...
    if captured_certs.is_empty() {
        return Err(ApiError::ExternalService("No certificate received during TLS handshake".to_string()));
    }
    ...
    let organization = extract_organization(&cert.subject)
