...
async fn test_scan_creation_load() {
    ...
    let results = run_load_test(
        app,
        config,
        |i| create_scan_request(format!("load-test-{}.com", i)),
    ).await;
    ...
    assert!(results.average_response_time < Duration::from_millis(500), "Average response time should be under 500ms");
}
...
async fn test_asset_listing_load() {
    ...
    assert!(results.requests_per_second > 15.0, "Should handle at least 15 requests per second");
    assert!(results.average_response_time < Duration::from_millis(300), "Average response time should be under 300ms");
}
...
async fn test_mixed_workload_load() {
    ...
    assert!(results.error_rate < 0.08, "Error rate should be less than 8% for mixed workload");
    assert!(results.requests_per_second > 8.0, "Should handle at least 8 requests per second");
    assert!(results.average_response_time < Duration::from_millis(800), "Average response time should be under 800ms");
}
...
async fn test_discovery_load() {
    ...
    assert!(results.error_rate < 0.15, "Error rate should be less than 15% for discovery");
    assert!(results.requests_per_second > 1.0, "Should handle at least 1 discovery request per second");
}
...
async fn test_evidence_upload_load() {
    ...
    let results = run_load_test(
        app,
        config,
        move |i| create_evidence_upload_request(&scan_ids_cloned[i % scan_ids_cloned.len()]),
    ).await;
    ...
    assert!(results.error_rate < 0.10, "Error rate should be less than 10% for file uploads");
    assert!(results.requests_per_second > 5.0, "Should handle at least 5 uploads per second");
    assert!(results.average_response_time < Duration::from_secs(2), "Average response time should be under 2 seconds");
}
    ...
    F: Fn(usize) -> Request<Body> + Send + Sync + 'static,
{
    ...
    for user_id in 0..config.concurrent_users {
        ...
        let task = tokio::spawn(async move {
            ...
            let delay = config.ramp_up_duration.mul_f64(user_id as f64 / config.concurrent_users as f64);
        ...
        let task = tokio::spawn(async move {
            ...
            for request_id in 0..config.requests_per_user {
                ...
                let request = match selected_type {
                    "scan_creation" => create_scan_request(format!("mixed-{}-{}.com", user_id, request_id)),
                    "asset_listing" => create_asset_list_request(),
                    "scan_listing" => create_scan_list_request(),
                    "seed_creation" => create_seed_request(format!("mixed-seed-{}-{}.com", user_id, request_id)),
...
async fn create_test_app() -> axum::Router {
    ...
    let test_config = Settings::new_with_env_file(false)
