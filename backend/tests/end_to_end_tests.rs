...
mod common;
use common::{create_test_app, extract_body, is_valid_datetime_string, is_valid_uuid_string};
...
async fn test_complete_seed_and_asset_workflow() {
    ...
    let seed_still_exists = final_list_json
        .as_array()
        .unwrap()
        .iter()
        .any(|seed| seed["id"].as_str().unwrap() == seed_id);
...
async fn test_risk_calculation_workflow() {
    ...
    for (cvss_base, asset_criticality, exploitability, expected_risk) in test_cases {
        ...
        assert!(
            (calculated_risk - expected_risk).abs() < 0.001,
            "Risk calculation mismatch: expected {}, got {}",
            expected_risk,
            calculated_risk
        );
        ...
        assert_eq!(
            json["components"]["asset_criticality_weight"]
                .as_f64()
                .unwrap(),
            asset_criticality
        );
        assert_eq!(
            json["components"]["exploitability_multiplier"]
                .as_f64()
                .unwrap(),
            exploitability
        );
...
async fn test_health_check_workflow() {
    ...
    for endpoint in health_endpoints {
        ...
        assert!(
            ...
            "Missing status in health check response for {}",
            endpoint
        );
    }
}
...
async fn test_concurrent_operations() {
    ...
    let listed_scan_ids: Vec<String> = list_json
        .as_array()
        .unwrap()
        .iter()
    ...
    for scan_id in &scan_ids {
        assert!(
            listed_scan_ids.contains(scan_id),
            "Scan {} not found in list",
            scan_id
        );
    }
}
...
async fn test_data_persistence() {
    ...
    for _ in 0..3 {
        ...
        assert!(is_valid_datetime_string(
            get_json["created_at"].as_str().unwrap()
        ));
        assert!(is_valid_datetime_string(
            get_json["updated_at"].as_str().unwrap()
        ));
