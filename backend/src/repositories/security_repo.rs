...
use crate::{
    error::ApiError,
    models::{
        SecurityScan, SecurityScanCreate, SecurityScanType, SecurityScanStatus, ScanTriggerType,
        SecurityFinding, SecurityFindingCreate, SecurityFindingUpdate, SecurityFindingFilter,
...
pub trait SecurityScanRepository: Send + Sync {
    ...
    async fn list_by_asset(&self, asset_id: &Uuid, limit: i64) -> Result<Vec<SecurityScan>, ApiError>;
    ...
    async fn update_status(&self, id: &Uuid, status: SecurityScanStatus) -> Result<SecurityScan, ApiError>;
    ...
    async fn get_latest_for_asset(&self, asset_id: &Uuid) -> Result<Option<SecurityScan>, ApiError>;
...
impl SecurityScanRepository for SqlxSecurityScanRepository {
    async fn create(&self, scan: &SecurityScanCreate) -> Result<SecurityScan, ApiError> {
        let id = Uuid::new_v4();
        let now = Utc::now();
        let scan_type = scan.scan_type.as_ref()
            .map(|t| t.to_string())
            .unwrap_or_else(|| "full".to_string());
        let trigger_type = scan.trigger_type.as_ref()
...
pub trait SecurityFindingRepository: Send + Sync {
    ...
    async fn create_or_update(&self, finding: &SecurityFindingCreate) -> Result<SecurityFinding, ApiError>;
    async fn get_by_id(&self, id: &Uuid) -> Result<Option<SecurityFinding>, ApiError>;
    async fn list_by_asset(&self, asset_id: &Uuid, limit: i64) -> Result<Vec<SecurityFinding>, ApiError>;
    async fn list_by_scan(&self, scan_id: &Uuid) -> Result<Vec<SecurityFinding>, ApiError>;
    async fn list_filtered(&self, filter: &SecurityFindingFilter) -> Result<(Vec<SecurityFinding>, i64), ApiError>;
    async fn update(&self, id: &Uuid, update: &SecurityFindingUpdate, updated_by: Option<Uuid>) -> Result<SecurityFinding, ApiError>;
...
impl SecurityFindingRepository for SqlxSecurityFindingRepository {
    ...
    async fn list_filtered(&self, filter: &SecurityFindingFilter) -> Result<(Vec<SecurityFinding>, i64), ApiError> {
        ...
        // Add ordering
        let order = format!(" ORDER BY {} {} LIMIT {} OFFSET {}", 
    ...
    async fn update(&self, id: &Uuid, update: &SecurityFindingUpdate, _updated_by: Option<Uuid>) -> Result<SecurityFinding, ApiError> {
        let now = Utc::now();
