...
mod tests {
        ...
        F: FnOnce() -> R,
    {
        ...
        let all_config_vars = vec![
            ...
            "ENABLE_URLSCAN",
            "ENABLE_OTX",
            "ENABLE_DNS_RECORD_EXPANSION",
            "ENABLE_WEB_CRAWL",
            "ENABLE_CLOUD_STORAGE_DISCOVERY",
            "ENABLE_WIKIDATA",
            "ENABLE_OPENCORPORATES",
            "RELATED_ASSET_CONFIDENCE_DEFAULT",
            "RATE_LIMIT_ENABLED",
            "RATE_LIMIT_REQUESTS",
    ...
    fn test_default_settings() {
        ...
        let result = with_env_vars(vec![], || Settings::new_with_env_file(false));
        ...
        assert_eq!(
            ...
            "postgresql://easm:easm@localhost:5432/easm"
        );
    ...
    fn test_validation_log_format() {
        let result = with_env_vars(vec![("LOG_FORMAT", "invalid")], || {
            Settings::new_with_env_file(false)
        });
    ...
    fn test_validation_positive_values() {
        ...
        with_env_vars(vec![("MAX_EVIDENCE_BYTES", "0")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });

        with_env_vars(vec![("HTTP_TIMEOUT_SECONDS", "0")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });

        with_env_vars(vec![("DNS_CONCURRENCY", "0")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });
    }
    ...
    fn test_validation_ranges() {
        ...
        with_env_vars(vec![("MAX_CIDR_HOSTS", "0")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });

        with_env_vars(vec![("MAX_CIDR_HOSTS", "25000")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });

        // Test max_discovery_depth range
        with_env_vars(vec![("MAX_DISCOVERY_DEPTH", "0")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });

        with_env_vars(vec![("MAX_DISCOVERY_DEPTH", "15")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });

        // Test confidence range
        with_env_vars(vec![("RELATED_ASSET_CONFIDENCE_DEFAULT", "-0.1")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });

        with_env_vars(vec![("RELATED_ASSET_CONFIDENCE_DEFAULT", "1.5")], || {
            let result = Settings::new();
            assert!(result.is_err());
        });
    }
    ...
    fn test_valid_log_formats() {
        with_env_vars(vec![("LOG_FORMAT", "json")], || {
            let settings = Settings::new().expect("Failed to create settings");
            assert_eq!(settings.log_format, "json");
        });

        with_env_vars(vec![("LOG_FORMAT", "plain")], || {
            let settings = Settings::new().expect("Failed to create settings");
