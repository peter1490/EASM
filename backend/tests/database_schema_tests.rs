...
async fn test_database_schema_compatibility() {
    ...
    let expected_tables = vec!["scans", "findings", "assets", "seeds", "evidence"];
...
async fn test_findings_table_schema() {
    ...
    for (col_name, _col_type) in expected_columns {
        assert!(
            columns.contains_key(col_name),
            "Missing column '{}' in findings table",
            col_name
        );
    }
}
...
async fn test_assets_table_schema() {
    ...
    for (col_name, _col_type) in expected_columns {
        assert!(
            columns.contains_key(col_name),
            "Missing column '{}' in assets table",
            col_name
        );
    }
}
...
async fn test_evidence_table_schema() {
    ...
    for (col_name, _col_type) in expected_columns {
        assert!(
            columns.contains_key(col_name),
            "Missing column '{}' in evidence table",
            col_name
        );
    }
}
...
async fn test_foreign_key_constraints() {
    ...
    assert!(
        scan_fk.is_some(),
        "Missing foreign key from findings.scan_id to scans.id"
    );
...
async fn test_enum_values_compatibility() {
    ...
    for status in valid_scan_statuses {
        ...
        assert!(
            result.is_ok(),
            "Failed to insert scan with status: {}",
            status
        );
    }
    ...
    for asset_type in valid_asset_types {
        ...
        assert!(
            result.is_ok(),
            "Failed to insert asset with type: {}",
            asset_type
        );
    }
}
...
async fn test_json_field_compatibility() {
    ...
    let retrieved_metadata: serde_json::Value =
        serde_json::from_str(&metadata_str).expect("Failed to parse JSON metadata");
