...
pub const COMMON_PORTS: &[u16] = &[
    21, 22, 23, 25, 53, 80, 110, 111, 135, 139, 143, 443, 445, 587, 993, 995, 1433, 1521, 3306,
    3389, 5432, 5900, 6379, 8080, 8443, 8888, 9000, 9090,
];

/// Extended ports for thorough scanning
pub const EXTENDED_PORTS: &[u16] = &[
    // Standard services
    7, 9, 13, 17, 19, 20, 21, 22, 23, 25, 26, 37, 53, 79, 80, 81, 82, 83, 84, 85, 88, 106, 110, 111,
    113, 119, 135, 139, 143, 144, 179, 199, 389, 427, 443, 444, 445, 465, 513, 514, 515, 543, 544,
    548, 554, 587, 631, 636, 646, 873, 990, 993, 995, 1025, 1026, 1027, 1028, 1029, 1110, 1433,
    1521, 1720, 1723, 1755, 1900, 2000, 2001, 2049, 2121, 2181, 2717, 3000, 3128, 3306, 3389, 3986,
    4899, 5000, 5001, 5003, 5009, 5050, 5051, 5060, 5101, 5190, 5357, 5432, 5631, 5666, 5800, 5900,
    5901, 6000, 6001, 6379, 6646, 7000, 7001, 7002, 7070, 8000, 8008, 8009, 8080, 8081, 8443, 8888,
    9000, 9001, 9042, 9090, 9100, 9200, 9300, 9418, 9999, 10000, 10443, 27017, 27018, 32768, 49152,
    49153, 49154,
];
...
pub const TOP_PORTS: &[u16] = &[
    ...
    6112, 6123, 6129, 6156, 6346, 6379, 6389, 6502, 6510, 6543, 6547, 6565, 6566, 6567, 6580, 6646,
    ...
    9080, 9081, 9090, 9091, 9099, 9100, 9101, 9102, 9103, 9110, 9111, 9200, 9207, 9220, 9290, 9300,
    ...
    10010, 10012, 10024, 10025, 10082, 10180, 10215, 10243, 10443, 10566, 10616, 10617, 10621,
    ...
    21571, 22939, 23502, 24444, 24800, 25734, 25735, 26214, 27000, 27017, 27352, 27353, 27355,
...
async fn grab_banner(
    ...
    if let Some(probe_data) = probe {
        if timeout(
            timeout_duration / 2,
            stream.write_all(probe_data.as_bytes()),
        )
        .await
        .is_err()
        {
...
fn get_service_probe(port: u16) -> Option<&'static str> {
    match port {
        80 | 8080 | 8000 | 8008 | 8888 | 9000 => Some("GET / HTTP/1.0\r\nHost: localhost\r\n\r\n"),
        ...
        for (category, signatures) in SERVICE_SIGNATURES.iter() {
            for sig in signatures {
                if banner.to_lowercase().contains(&sig.pattern.to_lowercase()) {
                            ...
                            if let Some(captures) = re.captures(banner) {
                                if let Some(version) = captures.get(1) {
                                    ...
                                    service_info.confidence =
                                        if sig.product.is_some() { 95 } else { 70 };
                                }
                            }
                        }
                    }
                    ...
                    service_info.cpe =
                        generate_cpe(product_for_cpe, service_info.version.as_deref());
...
pub async fn scan_ports_with_services(
    ...
    let results: Vec<PortScanResult> = stream::iter(ports.iter().cloned())
        .map(|port| {
            let ip = ip;
            async move { scan_port_with_banner(ip, port, timeout_duration).await }
        })
