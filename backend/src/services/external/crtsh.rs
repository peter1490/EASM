...
impl CrtShClient {
    ...
    pub async fn search_domain(&self, domain: &str) -> Result<Vec<String>, ApiError> {
        ...
        tracing::info!("Found {} unique subdomains for {}", valid_domains.len(), domain);
        Ok(valid_domains)
    }
    ...
    pub async fn search_organization(&self, organization: &str) -> Result<Vec<String>, ApiError> {
        if organization.trim().is_empty() {
            return Err(ApiError::Validation("Organization cannot be empty".to_string()));
        }
        ...
        let url = format!("https://crt.sh/?O={}&output=json", urlencoding::encode(organization.trim()));
    ...
    async fn search_crt_sh(&self, query: &str) -> Result<Vec<String>, ApiError> {
        ...
        let entries: Vec<CrtShEntry> = serde_json::from_str(&text)
            .map_err(|e| ApiError::ExternalService(format!("Failed to parse crt.sh response: {}", e)))?;
...
mod tests {
    ...
    async fn test_crtsh_domain_validation() {
        ...
        // Valid domains
        assert!(client.validate_and_clean_domain("example.com", "example.com").is_some());
        assert!(client.validate_and_clean_domain("sub.example.com", "example.com").is_some());
        
        // Invalid domains
        assert!(client.validate_and_clean_domain("*.example.com", "example.com").is_none());
        assert!(client.validate_and_clean_domain("", "example.com").is_none());
        assert!(client.validate_and_clean_domain("invalid domain", "example.com").is_none());
        assert!(client.validate_and_clean_domain("other.com", "example.com").is_none());
    }
