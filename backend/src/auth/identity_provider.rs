...
use openidconnect::{
    ...
    AuthenticationFlow, ClientId, ClientSecret, IssuerUrl, RedirectUrl,
};
...
impl OidcProvider {
    pub async fn google(settings: &Settings) -> Result<Self> {
        let client_id = ClientId::new(
            settings
                .google_client_id
                .clone()
                .context("Google Client ID not set")?,
        );
        let client_secret = ClientSecret::new(
            settings
                .google_client_secret
                .clone()
                .context("Google Client Secret not set")?,
        );
            ...
            CoreClient::from_provider_metadata(provider_metadata, client_id, Some(client_secret))
                .set_redirect_uri(RedirectUrl::new(
                    settings
                        .google_redirect_uri
                        .clone()
                        .context("Google Redirect URI not set")?,
                )?);
    ...
    pub async fn keycloak(settings: &Settings) -> Result<Self> {
        let client_id = ClientId::new(
            settings
                .keycloak_client_id
                .clone()
                .context("Keycloak Client ID not set")?,
        );
        let client_secret = ClientSecret::new(
            settings
                .keycloak_client_secret
                .clone()
                .context("Keycloak Client Secret not set")?,
        );
        ...
        let discovery_url = if let Some(url) = &settings.keycloak_discovery_url {
            ...
            return Err(anyhow::anyhow!(
                "Keycloak Discovery URL or Realm must be set"
            ));
        };
            ...
            CoreClient::from_provider_metadata(provider_metadata, client_id, Some(client_secret))
                .set_redirect_uri(RedirectUrl::new(
                    settings
                        ...
                        .clone()
                        .context("Keycloak Redirect URI not set")?,
                )?);
    ...
    pub async fn exchange_code(
        &self,
        code: String,
    ) -> Result<openidconnect::core::CoreIdTokenClaims> {
