...
impl SourceWeights {
    ...
    pub fn get_weight(&self, source: &str) -> f64 {
        match source.to_lowercase().as_str() {
            ...
            "otx" => self.otx,
            "org_search" | "organization" | "shodan_org_comprehensive" | "shodan_asn_comprehensive" => self.org_search,
...
impl MethodConfidence {
    pub const SEED: Self = Self { base: 1.0, description: "Direct seed input" };
    pub const SCAN_TARGET: Self = Self { base: 1.0, description: "Direct scan target" };
    pub const CIDR_EXPANSION: Self = Self { base: 0.85, description: "Direct IP ownership" };
    pub const DNS_RESOLUTION: Self = Self { base: 0.85, description: "Active DNS record" };
    pub const SHODAN_ASN: Self = Self { base: 0.8, description: "Shodan ASN member" };
    pub const TLS_CERT_WITH_ORG: Self = Self { base: 0.7, description: "Verified certificate" };
    pub const SHODAN_ORG: Self = Self { base: 0.7, description: "Shodan org match" };
    pub const SUBDOMAIN_ENUM: Self = Self { base: 0.6, description: "Subdomain enumeration" };
    pub const CRTSH_ORG: Self = Self { base: 0.6, description: "CT log organization" };
    pub const KEYWORD_SEARCH: Self = Self { base: 0.4, description: "Keyword match" };
    pub const TLS_CERT_NO_ORG: Self = Self { base: 0.3, description: "Unverified certificate" };
}
...
impl ConfidenceScorer {
    ...
    pub fn calculate_confidence(&self, factors: &ConfidenceFactors) -> f64 {
        ...
        if factors.sources.len() > 1 {
            ...
            let source_bonus = ((factors.sources.len() - 1) as f64 * self.multi_source_bonus * source_quality)
        ...
        if factors.distance_from_seed > 0 {
            let distance_penalty = (factors.distance_from_seed as f64 * self.distance_penalty_per_hop)
    ...
    pub fn calculate_domain_confidence(
        ...
        let base_confidence = source_list.iter()
    ...
    pub fn calculate_certificate_confidence(&self, has_organization: bool, sources: Vec<String>) -> f64 {
    ...
    fn calculate_source_quality(&self, sources: &[String]) -> f64 {
        ...
        let total_weight: f64 = sources.iter()
            .map(|s| self.source_weights.get_weight(s))
            .sum();
    ...
    fn has_cross_validation(&self, sources: &[String]) -> bool {
        let weights: Vec<f64> = sources.iter()
            .map(|s| self.source_weights.get_weight(s))
            .collect();
...
mod tests {
    ...
    fn test_multi_source_bonus() {
        ...
        let multi_source = ConfidenceFactors {
            base_confidence: 0.6,
            sources: vec!["crt.sh".to_string(), "shodan".to_string(), "dns_resolution".to_string()],
            ..Default::default()
        };
        ...
        assert!(multi_conf > single_conf, "Multi-source should have higher confidence");
    }
    ...
    fn test_subdomain_bonus() {
        ...
        assert!(sub_conf > non_sub_conf, "Direct subdomain should have higher confidence");
    }
    ...
    fn test_distance_penalty() {
        ...
        assert!(far_conf < close_conf, "Assets far from seed should have lower confidence");
    }
    ...
    fn test_scan_validation_bonus() {
        ...
        assert!(val_conf > unval_conf, "Validated assets should have higher confidence");
    }
    ...
    fn test_age_penalty() {
        ...
        assert!(fresh_conf > old_conf, "Fresh assets should have higher confidence than old ones");
    }
    ...
    fn test_merge_confidence() {
        ...
        assert!(merged >= 0.7, "Merged confidence should be at least the max of the two");
